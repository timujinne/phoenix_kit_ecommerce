...
- **The shop section in the catalogue's item and category forms matches
  the catalogue's labels and wording** (#61). daisyUI `.fieldset`
  wrappers (which set `font-size: .75rem`) made its labels 12px beside
  the catalogue's 14px ones; they are gone. The heading and card use the
  catalogue's 16px heading-with-icon and `shadow-lg`. Title Case labels
  that appear in both places are sentence case (`Product type`,
  `Compare at price`, `Cost per item`, the category visibility options).
- **The product form's category default is "— Category not set —"**, not
  "No category", which read as if there were none to pick. The product
  *page*'s "No category" is unchanged: there it states a fact about that
  product.

### Fixed

- **The shop-section visibility options (and four siblings) no longer
  ship as fuzzy guesses.** #61 lowercased the msgids but left them
  `fuzzy` with the shop category-form's "products" wording, so English
  still rendered "Active — Category and products visible". Elixir
  Gettext compiles fuzzy entries. All seven unfuzzied; trailing colons
  on `Download limit` / `Requires shipping` dropped; de/fr "products"
  aligned with the section's own "item" translations. `I18nTest` now
  fails a catalogue that still carries `fuzzy`.
